  ...
  for (int idim = 0; idim < dimension; idim++) {
    for (int idir = 0; idir < 2; idir++) {
      ...
      for (i = 0; i < noverlap; i++) {
        ...
        if (idim >= 1) {
          if (sbox_multi[0] == oboxlo[0]) sbox_multi[0] -= cutghostmulti[itype][0];
          if (sbox_multi[3] == oboxhi[0]) sbox_multi[3] += cutghostmulti[itype][0];
        }
        if (idim == 2) {
          if (sbox_multi[1] == oboxlo[1]) sbox_multi[1] -= cutghostmulti[itype][1];
          if (sbox_multi[4] == oboxhi[1]) sbox_multi[4] += cutghostmulti[itype][1];
        }
...
void CommCAC::reverse_comm_pair(Pair *pair)
{
  int i,irecv,n,nsend,nrecv;
...
void CommCAC::reverse_comm_fix(Fix *fix, int size)
{
  int i,irecv,n,nsize,nsend,nrecv;
...
void CommCAC::reverse_comm_fix_variable(Fix * /*fix*/)
{
  error->all(FLERR,"Reverse comm fix variable not yet supported by CommCAC");
}
...
void CommCAC::forward_comm_compute(Compute *compute)
{
  int i,irecv,n,nsend,nrecv;
...
void CommCAC::reverse_comm_compute(Compute *compute)
{
  int i,irecv,n,nsend,nrecv;
...
void CommCAC::forward_comm_dump(Dump *dump)
{
  int i,irecv,n,nsend,nrecv;
...
void CommCAC::reverse_comm_dump(Dump *dump)
{
  int i,irecv,n,nsend,nrecv;
...
void CommCAC::forward_comm_array(int nsize, double **array)
{
...
int CommCAC::exchange_variable(int n, double * /*inbuf*/, double *& /*outbuf*/)
{
  int nrecv = n;
  return nrecv;
}
...
void CommCAC::grow_swap_recv(int i, int n)
{
  delete [] recvproc[i];
