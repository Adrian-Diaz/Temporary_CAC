...
FixCAC_Harmonic::FixCAC_Harmonic(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
FixCAC_Harmonic::~FixCAC_Harmonic()
{
  if (copymode) return;
...
void FixCAC_Harmonic::init()
{
	...
	double *boxlo_bound = domain->boxlo_bound;
	double *boxhi_bound = domain->boxhi_bound;
	double origin[3];
	double distance;
	double distancex;
	double delr[3];
	...
	double cell = 20 * 3.80950;

	center_radius_sq = 2.25*56.25 * cell * cell;
	...
	origin[0] = (boxhi_bound[0] + boxlo_bound[0]) / 2;
	origin[1] = (boxhi_bound[1] + boxlo_bound[1]) / 2;
			...
			for (int j = 0; j < nodes_per_element; j++) {
				for (int l = 0; l < poly_count[i]; l++) {
					...
					if (distance<= center_radius_sq) {
						...
						nodal_positions[i][j][l][0] = nodal_positions[i][j][l][0] + Amplitude * cos(sqrt(distance)*wave_vector)*unit_vector[0];
						nodal_positions[i][j][l][1] = nodal_positions[i][j][l][1] + Amplitude * cos(sqrt(distance)*wave_vector)*unit_vector[1];
...
void FixCAC_Harmonic::setup(int vflag)
{
...
void FixCAC_Harmonic::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixCAC_Harmonic::post_force(int vflag)
{
  double **x = atom->x;
  double **v = atom->v;
  double **f = atom->f;
    ...
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
		  for (int j = 0; j < nodes_per_element; j++) {
			  ...
			  foriginal[1] += f[i][1];
			  foriginal[2] += f[i][2];
			  if (xstyle) nodal_forces[i][j][0] = xvalue;
			  if (ystyle) nodal_forces[i][j][1] = yvalue;
			  if (zstyle) nodal_forces[i][j][2] = zvalue;
		  }
      }
...
void FixCAC_Harmonic::post_force_respa(int vflag, int ilevel, int iloop)
{
...
void FixCAC_Harmonic::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double FixCAC_Harmonic::compute_vector(int n)
{
...
double FixCAC_Harmonic::memory_usage()
{
  double bytes = 0.0;
