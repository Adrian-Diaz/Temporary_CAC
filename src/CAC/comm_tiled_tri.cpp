...
void CommTiled::init()
{
 ...
 int maxswap = 6;
  if (mode == Comm::MULTI) {
   ...
   if (mode == Comm::CAC) {
    memory->create(cutghostCAC,atom->scale_count+1,3,"comm:cutghostCAC");
  }
...
if(!triclinic){
  ...
  cutghost[0] = cutghost[1] = cutghost[2] = cut;
    if (mode == Comm::MULTI) {
      ...
      for (i = 1; i <= ntypes; i++) {
        cut_test = 0.0;
        if (cutusermulti) cut_test = cutusermulti[i];
        cutghostmulti[i][0] = MAX(cut_test,cuttype[i]);
        cutghostmulti[i][1] = MAX(cut_test,cuttype[i]);
        cutghostmulti[i][2] = MAX(cut_test,cuttype[i]);
      }
    }
    ...
    if (mode == MULTI) {
      ...
      for (i = 1; i <= ntypes; i++) {
        cut_test = 0.0;
        if (cutusermulti) cut_test = cutusermulti[i];
        cutghostmulti[i][0] = length0 * MAX(cut_test,cuttype[i]);
        cutghostmulti[i][1] = length1 * MAX(cut_test,cuttype[i]);
        cutghostmulti[i][2] = length2 * MAX(cut_test,cuttype[i]);
      }
    }
  }
  ...
  for (int idim = 0; idim < dimension; idim++) {
    for (int idir = 0; idir < 2; idir++) {
      ...
      // overlap now has list of noverlap procs
