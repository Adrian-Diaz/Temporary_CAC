...
DumpCACStrain::DumpCACStrain(LAMMPS *lmp, int narg, char **arg) : Dump(lmp, narg, arg)
{
...
void DumpCACStrain::init_style()
{
  ...
  if (buffer_flag == 1) write_choice = &DumpCACStrain::write_string;
  else write_choice = &DumpCACStrain::write_lines;
...
int DumpCACStrain::modify_param(int narg, char **arg)
{
...
int DumpCACStrain::count()
{
	...
	for (int i = 0; i < nlocal; i++)
	{
		if (update->ntimestep - ptimestep == 0) {
			if (mask[i] & groupbit) m = m + nodes_per_element*poly_count[i] + 1;
		}
		else {
			if (mask[i] & groupbit) m = m + nodes_per_element*poly_count[i] + 1;
		}
...
void DumpCACStrain::write_header(bigint n)
{
  if (me == 0) {
...
void DumpCACStrain::pack(tagint *ids)
{
  int m,n;
  ...
  for (int i = 0; i < nlocal; i++) {
	  ...
	  for (int j = 0; j < nodes_per_element; j++) {
		  for (int k = 0; k < poly_count[i]; k++) {
			  ...
			  buf[m++] = nodal_positions[i][j][k][0];
			  buf[m++] = nodal_positions[i][j][k][1];
			  buf[m++] = nodal_positions[i][j][k][2];
		  }
		  }
	  }
  }
...
int DumpCACStrain::convert_string(int n, double *mybuf)
{
  int offset = 0;
  int m = 0;
  for (int i = 0; i < n; i++) {
...
void DumpCACStrain::write_data(int n, double *mybuf)
{
...
void DumpCACStrain::write_string(int n, double *mybuf)
{
...
void DumpCACStrain::write_lines(int n, double *mybuf)
{
  int m = 0;
  for (int i = 0; i < n; i++) {
    fprintf(fp,format,
...
double DumpCACStrain::shape_function(double s, double t, double w, int flag, int index) {
...
double DumpCACStrain::shape_function_derivative(double s, double t, double w, int flag, int index, int derivative) {
	double shape_function = 0;
